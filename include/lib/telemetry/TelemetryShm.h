...
private:
    ...
    std::unique_ptr<std::unique_ptr<Shm>[]> write_locks; // locks used for locking individual writes to packets
    std::unique_ptr<bool[]> locked_packets; // which packets do we currently have locked
    std::unique_ptr<std::unique_ptr<Shm>[]> packet_blocks; // list of blocks holding raw telemetry data
    std::unique_ptr<std::unique_ptr<Shm>[]> info_blocks; // list of blocks holding uint32_t nonces TODO rename this to something better lol
